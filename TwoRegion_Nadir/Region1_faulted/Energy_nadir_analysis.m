        ...
        for R=P_loss-D_prime*0.5:500:P_loss-D_prime*0.5+4e3
            ...
            for fraction_R=0.25:0.25:0.75
                ...
                for fraction_H=0.25:0.125:0.75
                    ...
                    nadir1_check = abs(min(Delta_f1.Data));
                    i=1;
                    while nadir1_check >= Delta_fmax
                        % Now get the value of k* in constraint (13) of paper "Stochastic 
                        % Scheduling With Inertia-Dependent...", so that I get a value of R
                        % close to the amount needed for barely complying nadir in single-area 
                        syms k
                    ...
                    nadir1(j) = nadir1_check;
                    [~,t_nadir_index] = min(Delta_f1.Data);
                    t_nadir1(j) = Delta_f1.Time(t_nadir_index);
            ...
            %                     single_area_samples = single_area(Delta_f1.Time);
                    ...
                    t_until_nadir = Delta_f1.Time(1:t_nadir_index);
                    Delta_f1_until_nadir = Delta_f1.Data(1:t_nadir_index);
                    
                    Delta_f2_until_nadir_in_region1 = Delta_f2.Data(1:t_nadir_index);
                    
                    %integral1 = trapz(t_until_nadir,Delta_f2_until_nadir);
                    
                    % CHANGE SIGN of Delta_f for the integrals:
                    Delta_f1_until_nadir = -Delta_f1_until_nadir;
                    Delta_f2_until_nadir_in_region1 = -Delta_f2_until_nadir_in_region1;
                    
                    
                    % Single and double integrals for Delta_f1
                    first_integral_f1 = [];
                    for k=2:length(Delta_f1_until_nadir)
                        first_integral_f1 = [first_integral_f1 trapz(t_until_nadir(1:k),Delta_f1_until_nadir(1:k))];
                    end
                    double_integral_f1 = [];
                    ...
                    integral1(j) = first_integral_f1(end);
                    
                    % Single and double integrals for Delta_f2 (only the
                    % double integral is needed, but the single integral
                    % must be computed to compute the double)
                    first_integral_f2 = [];
                    for k=2:length(Delta_f2_until_nadir_in_region1)
                        first_integral_f2 = [first_integral_f2 trapz(t_until_nadir(1:k),Delta_f2_until_nadir_in_region1(1:k))];
                    end
                    double_integral_f2 = [];
                    ...
                    integral2(j) = -(double_integral_f2(end) - double_integral_f1(end)); % I need the energy imported to this region, that's why the negative sign
